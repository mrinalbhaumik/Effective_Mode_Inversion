...
[filepath, name, ext]   = fileparts('Example_Target2_24Ch@1m_15m_Offset.txt'); 
...
Acquisition.r0          = 15;           % source to 1st sensor distance (m)
...
SwarmCount              = 50;           % Number of particles (swarm size: 50-100 is good number)
...
Layer_param.h_count     = [3 4];        % If 'h' is 'variable': [LRs: Ex. 1.5 2 3], [LNs: Ex. 4 5... + H.S]; If 'fixed': [LNs] as the number of layers
Layer_param.Vs          = 'variable';   % Always: 'variable'
Layer_param.Vs_range    = [90 700];       % numeric bounds:Ex.[150 800] or use: 'Auto'
...
Layer_param.Vs_reversal = 0.8;          % allowable fractional Vs drop (if any)
Layer_param.Vs_halfspace_max = 'yes';   % Half-space velocity allways maximum, 'yes'/'no'
Layer_param.DCR_range   = [4 4.5];        % DCR: Depth conversion ratio, D_max = max_wavelength / DCR;
% Read description ---------------------------------------
...
for ii = 1 : numel(Layer_param.h_count)
    ...
    for trial = Trial_num_index
        ...
        options = optimoptions('particleswarm','SwarmSize',SwarmCount, 'MaxIterations', Max_itr,...
            'PlotFcn',@PSO_fun, 'InitialSwarmMatrix', init_var,'MinNeighborsFraction',0.8,'UseParallel', true,...
